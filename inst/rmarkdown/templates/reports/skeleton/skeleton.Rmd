...
tabla_total <- get_selected_table(tables, INDICADOR)

tabla_total <- clean_historic_data(dataset = tabla_total)
tabla_epiTime_period <- getTable_hitoric_epi_times_period(dataset = tabla_total)


#Perido epidemiologico que va a mostrar el "ULTIMO año"
periodo_epi <- 8
#Filtra los años :OJO (revisar logica)
tabla_epiTime_period <- tabla_epiTime_period %>%
filter((tabla_epiTime_period$anos == 2024 & periodo_epidemiologico <= periodo_epi) | tabla_epiTime_period$anos != 2024)


# Prepare data for stacked bar and line charts
historic_epi_times <- get_historic_epi_times(tabla = tabla_epiTime_period)
...
plot_historic_epi_time(df=tabla_epiTime_period, bars_df=stacked_data, lines_df=line_data, periodo_epi = 8 )
